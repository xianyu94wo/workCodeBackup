...
class DataQuery(Ice.Application):
    ...
    def __init__(self, serverIp="10.96.89.55", serverPort=1888, configFile="client.config"):
        '''
        Constructor
        '''
    ...
    def callAPI_to_array2D(self, userId, pwd, interfaceId, params):
        if self.api:
            retArray2D = self.api.callAPItoarray2D(userId, pwd, \
                                                   interfaceId, self.clientIp, self.language, self.version, \
                                                   params)
            return retArray2D

    def callAPI_to_gridArray2D(self, userId, pwd, interfaceId, params):
        if self.api:
            retGridArray2D = self.api.callAPItogridArray2D(userId, pwd, \
                                                           interfaceId, self.clientIp, self.language, self.version, \
                                                           params)
            return retGridArray2D

    def callAPI_to_saveAsFile(self, userId, pwd, interfaceId, params, dataFormat, savePath):
        if self.api:
            result = self.api.callAPItosaveAsFile(userId, pwd, \
                                                  interfaceId, self.clientIp, self.language, self.version, \
    ...
    def callAPI_to_fileList(self, userId, pwd, interfaceId, params):
        if self.api:
            result = self.api.callAPItofileList(userId, pwd, \
                                                interfaceId, self.clientIp, self.language, self.version, \
                                                params)
            return result

    def callAPI_to_downFile(self, userId, pwd, interfaceId, params, fileDir):
    ...
    def callAPI_to_serializedStr(self, userId, pwd, interfaceId, params, dataFormat):
        if self.api:
            result = self.api.callAPItoserializedStr(userId, pwd, \
                                                     interfaceId, self.clientIp, self.language, self.version, \
                                                     params, dataFormat)
            return result

    # store
    def callAPI_to_storeArray2D(self, userId, pwd, interfaceId, params, inArray2D):
        if self.api:
            result = self.api.callAPItostoreArray2D(userId, pwd, \
                                                    interfaceId, self.clientIp, self.language, self.version, \
                                                    params, inArray2D)
            return result

    def callAPI_to_storeFileByFtp(self, userId, pwd, interfaceId, params, inArray2D, ftpfiles):
        if self.api:
            result = self.api.callAPItostoreFileByFtp(userId, pwd, \
                                                      interfaceId, self.clientIp, self.language, self.version, \
                                                      params, inArray2D, ftpfiles)
            return result

    def callAPI_to_storeSerializedStr(self, userId, pwd, interfaceId, params, inString):
        if self.api:
            result = self.api.callAPItostoreSerializedStr(userId, pwd, \
                                                          interfaceId, self.clientIp, self.language, self.version, \
                                                          params, inString)
            return result
